...
| `api/test_api.c` | api test | libnvmpi public C API smoke: create/close lifecycle for all decoder codecs and encoder, rapid lifecycle ×50 (no FFmpeg required; build with `-DBUILD_TESTING=ON`) |

Remaining feature-gated suites (JPEG, encoder level, encoder bitrate, lossless
overflow) ship with their respective feature PRs. See individual issues for
tracking.
